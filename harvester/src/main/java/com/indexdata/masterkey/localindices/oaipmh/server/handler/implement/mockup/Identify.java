...
public class Identify extends CommonOaiPmhHandler {
  ...
  public OaiPmhResponse handle(OaiPmhRequest request) 
  {
    ...
    xml.append(getRequest(request)).append(getElement(request));
    xml.append(getIdentify(request)).append(getElementEnd(request));
    
    return new MockupOaiPmhResponse(xml.toString()); 
  }

  private Object getIdentify(OaiPmhRequest request) {
