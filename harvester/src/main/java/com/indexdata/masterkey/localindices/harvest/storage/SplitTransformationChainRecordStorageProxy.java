...
public class SplitTransformationChainRecordStorageProxy extends RecordStorageProxy 
{
	...
	public SplitTransformationChainRecordStorageProxy(final RecordStorage storage, final XMLReader xmlFilter) throws IOException,
		...
		thread = new Thread(new Runnable() {
			...
			private void processDataFromInputStream(PipedInputStream input) {
				try {
					...
					transformException = new TransformerException("IO Error while parsing/transforming: " + ioe.getMessage(), ioe);
					ioe.printStackTrace();
